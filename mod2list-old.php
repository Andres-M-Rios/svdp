<!DOCTYPE html>
<?php
// change the date to the form acceptable in Filemaker Pro: mm/dd/yyyy (leading zeroes optional)
function mydate($m,$d,$y)
{	
	$month = array( 1=>"Jan",2=>"Feb",3=>"Mar",4=>"Apr",5=>"May",6=>"Jun",7=>"Jul",8=>"Aug",9=>"Sep",10=>"Oct",11=>"Nov",12=>"Dec");
	if ($d<1 ||$d>31) return " ";
	if ($y < 2000 || $y > 2020) return " ";
	for ($i=1;$i<13;$i++)
		if($m==$month[$i]) $m=$i;
	if ($m<1 || $m>12) return " ";
	return $m."/".$d."/".$y;
...
$latest = $_POST["latest"];
// this is the latest repeating section, which may be added if valid, or ignored if not valid
...
$line .= $latest."\t";
//the chr(29) is only BETWEEN repeating fields, NOT before, so remove the first character.
